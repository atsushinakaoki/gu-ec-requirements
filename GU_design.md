...
本書は、要件定義書の要件をそのまま実現するのではなく、①の範囲において簡略化した
箇所がある。該当箇所を以下に明示する。

| 対象 | 要件定義書の定め | 本書における扱い | 理由 |
|---|---|---|---|
| 認証基盤 | ユニクロとの共通認証基盤を利用する（FR-591-04、SEC-01） | EC Backend が member テーブルのパスワードを照合し、JWT を発行する | 共通認証基盤は外部システムであり、本書の対象範囲外（2.2）。①では当該基盤を模した実装とする |
| 2段階認証 | 提供する（FR-591-02、SEC-02） | ①では実装しない | 上記に同じ |

**この簡略化は、要件を満たしていないことを意味する。** ②以降または実運用においては、
共通認証基盤への接続に置き換える必要がある。

なお、5.3（認証）に記述する設計事項のうち、JWT の保持方法、Cookie の属性、パスワード
のハッシュ化に関するものは、認証を自前で行うか外部基盤に委ねるかにかかわらず適用
される。
...
| min_alteration_length_mm | int | ○ | すそ上げ可能な最短の丈（mm）。alterable = true の場合に必須 |
| created_at | datetime | × | |
| updated_at | datetime | × | |

**すそ上げ可能な丈の範囲を、商品ごとに保持する。**
...
一律の下限（例：400mm）を定めない。商品により加工可能な最短の丈が異なるためである。
KIDS 商品のように元の丈が短い商品では、成人向けを前提とした下限を適用すると、有効な
丈が存在しないという矛盾が生じる。

| ID | 設計事項 |
|---|---|
| DS-311 | すそ上げ可能な丈の下限を、商品ごとに保持する |
| DS-312 | alterable = true の商品について、min_alteration_length_mm <= original_length_mm であることを、商品マスタの登録時に検証する |
...
| quantity | int | × | 実在庫数。CHECK 制約により 0 以上とする |
| updated_at | datetime | × | |
...
| ID | 設計事項 |
|---|---|
| DS-321 | quantity に CHECK (quantity >= 0) 制約を設ける |
| DS-322 | 負の値を読み出した場合、データ不整合として例外を発生させ、引当を行わない |
| DS-323 | 当該事象を ERROR としてログに記録する |

DS-322 について。負の在庫を 0 として扱う方式、および計算結果をそのまま返す方式は
採らない。いずれもデータの破損を通常の在庫切れとして扱うことになり、障害が検知
されない。
...
| 操作 | 現在の状態 | 挙動 |
|---|---|---|
| confirm() | ACTIVE | 成功。CONFIRMED へ遷移 |
| confirm() | CONFIRMED / RELEASED | **例外。状態を変更しない** |
| release() | ACTIVE | 成功。RELEASED へ遷移 |
| release() | RELEASED | **何もしない（冪等）** |
| release() | CONFIRMED | **例外。状態を変更しない** |

| ID | 設計事項 |
|---|---|
| DS-331 | confirm() は status = ACTIVE の場合のみ成功し、それ以外は例外を発生させる |
| DS-332 | release() は status = RELEASED に対して冪等とし、例外を発生させない |
| DS-333 | release() は status = CONFIRMED に対して例外を発生させる |

**confirm() と release() で厳格さを分ける理由**

confirm() は注文確定の直前に呼ばれる。RELEASED の引当に対して成功させると、在庫を
確保していない状態で注文が成立する。何もせず無視した場合、呼び出し側は成功したと
みなして処理を続けるため、注文と在庫の状態が食い違う。

一方 release() は、期限切れの解放処理（3.4.4）から呼ばれる。この処理は失敗時に
再実行されうるため、すでに RELEASED のものに対して呼ばれることが正常な事象として
生じる。ここで例外を発生させると、処理が停止する。

| status | 意味 | stock.quantity | 引当可能数への影響 |
|---|---|---|---|
| ACTIVE | カート投入により確保中 | 減算しない | **減算する** |
| CONFIRMED | 注文確定により確定 | **減算済み** | 減算しない |
| RELEASED | 解放済み | 減算しない | 減算しない |

CONFIRMED の行を削除せず残す設計とした。誰がいつ引き当てたかの記録を保持するため
である。

**CONFIRMED を引当可能数の算出から除外する。** 注文確定時に stock.quantity を減算
しているため、あわせて CONFIRMED も減算すると、同一の数量を二重に差し引くことに
なる（3.4.3 参照）。
...
| status | varchar(20) | × | PENDING_PAYMENT / CONFIRMED / FAILED（4.4.3） |
...
| idempotency_key | varchar(64) | × | 冪等キー。UNIQUE 制約を付す（4.4.8） |
| content_fingerprint | varchar(64) | × | 注文内容を正規化したハッシュ値（DS-460） |
...
| ID | 設計事項 |
|---|---|
| DS-391 | 消費税は、税抜の合計金額（商品・加工料・送料・手数料）に税率を乗じて算出する |
| DS-392 | 1円未満を切り捨てる |
| DS-393 | 端数処理は注文単位で1回のみ行い、明細ごとには行わない |

DS-393 について。明細ごとに端数処理を行うと、明細数により総額が変動する。注文単位で
1回とすることで、算出結果が一意に定まる。
...
注文確定時に stock.quantity を減算する（4.4.2）。この時点で、確定した数量は在庫数
そのものから差し引かれている。

仮に CONFIRMED も減算すると、以下のとおり同一の数量を二重に差し引くことになる。

| 時点 | stock.quantity | ACTIVE | CONFIRMED | 引当可能数（誤） | 引当可能数（正） |
|---|---|---|---|---|---|
| 初期 | 10 | 0 | 0 | 10 | 10 |
| 1点をカート投入 | 10 | 1 | 0 | 9 | 9 |
| 注文確定 | 9 | 0 | 1 | **8** | **9** |

CONFIRMED の行を残す設計であるため、この誤差は一時的なものではなく、確定した注文の
数だけ累積する。

| ID | 設計事項 |
|---|---|
| DS-341 | 引当可能数の算出において、CONFIRMED の reservation を減算しない |
| DS-342 | 在庫の減算は注文確定時の1回のみとし、二重に減算しない |
...
「同一の明細」は SKU のみでは判定しない。以下の組み合わせによる。

```
sku_id + alteration_type + alteration_length_mm
```

| 会員カート | 未ログインカート | 判定 |
|---|---|---|
| 品番A・M・加工なし | 品番A・M・加工なし | 同一 |
| 品番A・M・丈700mm | 品番A・M・丈700mm | 同一 |
| 品番A・M・丈700mm | 品番A・M・丈650mm | **別の明細** |
| 品番A・M・加工なし | 品番A・M・丈700mm | **別の明細** |

SKU が同一でも、すそ上げの内容が異なれば別の商品である。同一のパンツを2本購入し、
それぞれ異なる丈に加工することは通常の購買行動である。これを同一とみなして一方を
破棄すると、顧客の購買意図を損なう。

| ID | 設計事項 |
|---|---|
| DS-424 | カートの統合において、同一の明細の数量を合算しない |
| DS-428 | 明細の同一性を、sku_id・alteration_type・alteration_length_mm の組み合わせにより判定する |
...
| DS-459 | 冪等キーを order テーブルに保持し、UNIQUE 制約により重複を排除する |
| DS-460 | 注文内容を正規化したハッシュ値（fingerprint）を保持し、冪等キーと併せて同一性を判定する |
| DS-461 | 同一の冪等キーで fingerprint が異なる要求に対し、422 を返す |

冪等キーは、確認画面の表示時に Backend が発行し、Frontend が保持する。

**DS-459 について**

同時に2つの要求が到達した場合、アプリケーション側で「既に存在するか」を確認してから
挿入する方式では、両者が確認を通過しうる。DB の UNIQUE 制約により、後続の挿入を
確実に失敗させる。

**DS-460 について**

注文の作成要求（POST /api/orders）の本文は expectedTotal のみであり、明細・配送先・
支払方法はサーバー側のカートから取得する（6.4.5）。したがって要求の本文を比較する
だけでは、内容の同一性を判定できない。

| 状況 | 本文の比較 | 実際 |
|---|---|---|
| 同一キー、カートの内容が変わっていない | 一致 | 同一の注文 |
| 同一キー、カートの内容が変わったが金額が同じ | **一致** | **別の注文** |

このため、注文内容を正規化したハッシュ値を保持する。対象は以下とする。
...
| 冪等キー | fingerprint | 処理の状態 | 応答 |
|---|---|---|---|
| 同一 | 同一 | 完了 | 最初の結果を返す |
| 同一 | 同一 | 処理中 | 409 Conflict |
| 同一 | **異なる** | — | **422 Unprocessable Entity** |
| 異なる | — | — | 新規の注文として処理 |

同一のキーで内容が異なる要求は、通信の再送ではない。Frontend の不具合、または要求の
改ざんの可能性がある。最初の結果を返すと、顧客は変更後の内容で注文したと認識した
まま、変更前の内容で決済されることになる。
...
| 数量（1明細あたり） | 整数 | 1 | 10 | |
| カートの明細数 | 整数 | 0 | 100 | |
| 仕上がり丈 | 整数（mm） | 商品ごと（min_alteration_length_mm） | 商品ごと（original_length_mm） | 5mm刻み |
...
| 項目 | 内容 |
|---|---|
| 決めた数字 | 下限・上限とも商品ごとに設定。刻みは5mm |
| 下限 | product.min_alteration_length_mm |
| 上限 | product.original_length_mm |
| 根拠 | 加工可能な最短の丈は、商品の構造およびデザインにより異なる |

**一律の下限を定めない理由**

当初、成人向けのボトムスを前提として 400mm を下限とすることを検討した。しかし
KIDS 商品のように元の丈が 400mm 未満の商品が存在する場合、有効な丈が存在しない
という矛盾が生じる。

商品ごとに下限を保持することで、この矛盾を回避する（3.3.1、DS-311）。

| ID | 設計事項 |
|---|---|
| DS-711 | 仕上がり丈が、当該商品の元の丈（product.original_length_mm）を超える場合を不正とする |
| DS-712 | 5mm 刻みでない値を不正とする |
| DS-716 | 仕上がり丈が、当該商品の下限（product.min_alteration_length_mm）を下回る場合を不正とする |
